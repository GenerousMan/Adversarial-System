...
class Model():
    ...
    def predict(self,images,flag=True):
        images = images*255. - tf.constant([123.68, 116.779, 103.939])
...
if __name__=="__main__":
  ...
  images=images/255.0
  labels = np.load("../../../datas/label/1000-vggres-same-label.npy").astype(np.int)
  ...
  for k in ks:
    for ep in ep_s:
      with tf.Session(config=config) as sess:
          ...
          imgs_adv = igsm(model, X, lab, epochs=k, eps=ep, clip_min=0., clip_max=1.,min_proba=2.0)
          ...
          np.save("../../../datas/image/IGSM-Res50-Slim-"+str(k)+"-"+str(int(ep*255.))+".npy",adv_images)
          f=open("./accuracy_IGSM.txt","a+")
          f.write("0-1, Res50, "+str(k)+", "+str(ep)+", acc, "+str(cur_accuracy2)+"\n" )
          f.close()
          ...
          for i in range(10):
            ...
            img.save("./test/IGSM_0-Res50-"+str(k)+"_"+str(i)+".png")
            Pic_each=(images[i]+0.)*255.0
            #img = Image.fromarray(Pic_each.astype('uint8'))

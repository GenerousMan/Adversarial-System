...
if __name__=="__main__":
  ...
  #attack_labels=np.load("/home/nesa320/Ji_3160102420/new_dataset/RES-152-LL-LABEL.npy").astype(int)
  ...
  lab_onehot = tf.placeholder(tf.int32, [batch_size, image_class])
  ...
  for ep in ep_s:
    with tf.Session(config=config) as sess:
        ...
        #
        # adv_images = attack.attack(images, lab)
        

        imgs_adv=Tmi_fgsm(model,X,lab_onehot,y_target=False,eps=ep)
        ...
        for i in range(epochs_num):
          ...
          adv = sess.run([imgs_adv], feed_dict={X: imgs_batch,
                                                lab_onehot:labs_batch})
        
          # for k in range(10):
          #   Pic_each=(adv[0][k]+0.)*255.0
          #   cv.imwrite("./test/U-MI-FGSM_0-1-"+str(k)+"_"+str(ep)+".png",Pic_each)
        ...
        np.save("/home/nesa320/Ji_3160102420/other_model_result/UTMI-Res50-Slim-"+str(ep*255.)+".npy",adv_images)
        f=open("/home/nesa320/Ji_3160102420/modified_result/accuracy_UTMI.txt","a+")
        f.write("0-1, res50, "+str(ep*255.)+", acc, "+str(cur_accuracy2)+"\n" )
        f.close()
        ...
        #   img.save("./modified_result/test/FGSM_0-1-"+str(k)+"_"+str(i)+".png")

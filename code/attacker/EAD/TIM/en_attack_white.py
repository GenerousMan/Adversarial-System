...
def predict2(model, images, batch_size):
    # print("before feature")
    feature = extract_feature(model, images, batch_size)
    print("feature.shape=", feature.shape)
    # print("before resofdet")
    resofdet = tf.py_func(getResultofdector, [feature, batch_size], tf.float32)
    print("resofdet.shape=", resofdet.get_shape())
    # print("before logit")
    logit = model.predict(images, True)
    print("logit.shape=", logit.shape)
    # print("before temp")
    temp = 2 * resofdet * tf.reshape(tf.reduce_max(logit, 1), [batch_size, 1])
    print("temp.shape=", temp.shape)
    # print("before output")
    output = tf.concat([logit, temp], 1)
    print("output.shape=", output.shape)
    return output


def getResultofdector(features, batch_size):
    detector = joblib.load("./ML_models/model_RF.m")
    features = features.astype(np.float64)
    resofdet = detector.predict_proba(features)
    resofdet = np.reshape(resofdet[:, 1], [batch_size, 1])
    resofdet = resofdet.astype(np.float32)
    # print("resofdet is: ", str(resofdet))
    return resofdet
...
class EADEN:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        image_size, num_channels, num_labels = 224, 3, 1001
        self.sess = sess
        ...
        self.output = predict2(model,self.newimg,self.batch_size)
        self.output_y = predict2(model,self.slack,self.batch_size)
        #1001

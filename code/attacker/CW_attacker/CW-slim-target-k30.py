...
if __name__=="__main__":
  ...
  with tf.Graph().as_default():
      ...
      with tf.Session(config=config) as sess:
          ...
          CW = CarliniL2(sess, model, batch_size=batch_size, confidence=30, targeted=True, max_iterations=1000)
          ...
          for i in range(epochs_num):
                ...
                print (np.max(adv),np.min(adv))
                np.save("./results/CW-target-slim-k30-test-adv-max150",adv)
                for j in range(0, batch_size):
                    temp[i * batch_size + j] = adv[j]
                print (np.max(temp),np.min(temp))

          np.save("./results/CW-target-slim-k30-test",temp)
